...
class NegativeSampling_common(Strategy):
	...
	def forward(self, data):
		score_trans, score_feature = self.model(data)
		p_score_trans = self._get_positive_score(score_trans)
		n_score_trans = self._get_negative_score(score_trans)
		loss_res = self.loss(p_score_trans, n_score_trans) + score_feature.mean()
